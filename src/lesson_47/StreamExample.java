...
1. Stream (поток) - поток данных это последовательность элементов.
Поток даёт возможность получать поочерёдно элементы для обработки.
...
Промежуточные операции (Intermediate operations) - это операции которые преобразуют другой поток в другой поток (возвращют поток)
...
Ленивые стримы:
Стримы не выполняют прмежуточные операции, пока на потоке не будет вызван терминальный метод
